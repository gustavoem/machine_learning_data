...
# 1st quartile:   0-150
# 2nd quartile: 151-302
# 3rd quartile: 303-454
# 4th quartile: 455-605

foreach my $index (0..($n-1))
{
  my @order_statistics = sort {$a <=> $b} @{$statistics[$index]};

  ${quartile{$index}->[1]} = $order_statistics[150];
  ${quartile{$index}->[2]} = $order_statistics[302];
  ${quartile{$index}->[3]} = $order_statistics[454];
}
